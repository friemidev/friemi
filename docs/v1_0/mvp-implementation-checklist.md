...
- 第一版好友系统只支持通过好友号或用户昵称精确查找，不做邮箱反查、通讯录导入、推荐好友、附近的人。
...
```text
feature/chat-notification-polish
```

目标：

```text
把聊天、通知和活动变更提醒整理成一个清晰、克制、可快速处理的沟通体验
```
...
- 不做复杂通知规则设置
- 不做群聊通知和全站公告
- 优先优化站内通知、顶部铃铛、通知中心和聊天列表可读性
- 通知要帮助用户处理活动和好友沟通，不要制造噪音

小功能：

- [x] 聊天列表保留好友面板逻辑，移动端直接显示好友列表，桌面端右侧显示好友 / 聊天入口
- [x] 聊天列表行展示头像、昵称、最后消息、时间和轻量活动信号，避免活动信号抢占最后消息层级
- [x] 空好友、空聊天、只读聊天状态使用面向用户的短文案，不出现开发视角描述
- [x] 添加好友入口使用图标按钮 + 弹窗，弹窗展示自己的好友号并支持复制
- [x] 添加好友弹窗优先展示自己的好友号和添加表单，收到的申请放在下方处理
- [x] 用户未设置昵称时先引导设置昵称，不直接展示 Google 姓名或邮箱作为公开昵称
- [x] 好友申请和报名审核列表不展示用户邮箱，统一用昵称 / 好友号识别
- [x] 移动端添加好友弹窗使用真正全屏布局，避免出现半屏浮层和内容溢出
- [x] 桌面端 Header 和聊天页面宽度适配中英法三语，按钮和品牌名不换行
- [x] 聊天输入仅支持纯文本，提供固定 emoji 表情弹层，不支持图片、附件和富文本
- [x] 聊天输入接近字数上限时才显示计数，避免默认解释文案占位
- [x] 聊天自动刷新不打断正在输入或已有草稿的消息
- [x] 移动端聊天详情去掉框中框观感，桌面端聊天区域尽量占满可用高度
- [x] 聊天详情头部居中展示对方昵称，并提供更多菜单进入对方主页
- [x] 公开个人页只展示公开资料、发起活动和关注关系，不暴露非本人的完整报名记录
- [x] 发送好友申请成功后保留弹窗反馈，并刷新好友 / 消息页面数据
- [x] 好友列表和好友申请查询不再向前端传递邮箱字段，前端只用昵称 / 好友号识别用户
- [x] 关注 / 粉丝弹层和聊天空状态移除英文硬编码与过重边框，保持三语一致
- [x] 添加好友入口优先使用好友号 / 昵称，6 位好友号查不到时禁止误提交
- [x] 好友预览和发送申请使用同一套查找规则，避免预览失败但仍能发送
- [x] 好友号支持去除空格 / 连字符后匹配，预览接口异常时给出可见反馈
- [x] 收到的好友申请卡片突出对方信息和操作按钮，移动端更易处理
- [x] 好友预览输入时立即显示查找状态，避免短暂误报“未找到”
- [x] 好友申请卡片隐藏无意义空简介，优先展示好友号、留言和处理按钮
- [x] 顶部通知铃铛显示未读数量，未读数量变化不挤压 Header 布局
- [x] 通知中心按时间倒序展示通知，未读通知视觉层级高于已读通知
- [x] 通知项展示清晰来源：报名、审核、活动取消、活动更新、好友申请、聊天相关提醒
- [x] 通知项可以跳转到对应活动、报名审核区、好友申请或聊天页
- [x] 好友申请通知进入消息页时自动展开处理弹窗
- [x] 支持单条通知标记已读
- [x] 支持一键标记全部已读
- [x] 活动发起人收到报名待审核通知
- [x] 报名用户收到审核通过 / 拒绝通知
- [x] 活动取消时，已报名用户收到活动通知
- [x] 活动关键字段变更时，已报名用户收到活动通知
- [x] 避免重复通知：同一活动同一用户同一事件不要被重复创建
- [x] 用户自己的操作不要给自己制造无意义通知
- [x] 移动端通知列表触控区域足够大，标题和按钮不换行
- [x] 三语文案同步维护 `zh-CN / en / fr`

验收标准：

- 用户能在 Header 看到是否有未读通知
- 点击通知铃铛可以进入通知中心
- 通知中心能清楚区分未读和已读
- 报名、审核、活动取消 / 更新、好友申请等关键事件会生成站内通知
- 通知点击后能到达正确页面或处理入口
- 聊天列表和通知列表在 iPhone 宽度下没有横向溢出
- 法语版 Header 和主要按钮不出现不合理换行
- `npm run typecheck` 通过

实现注意事项：

- 通知创建逻辑应集中在对应 server action / 服务函数里，不要只靠前端触发。
- 通知表如果已有类型枚举，新增类型要同步更新 Prisma、查询函数和三语文案。
- 通知去重建议使用业务唯一键，例如 `recipientId + type + activityId + actorId + eventKey`。
- 通知列表查询要限制数量，避免一次加载过多历史通知。
- 聊天相关通知第一版可以克制处理：优先更新聊天列表，不必每条消息都生成强通知。
- 活动更新通知第一版只覆盖关键字段：取消、时间、地点、报名审核结果。
- 移动端优先保证信息密度和可点性，不要把通知解释文案写得过长。
- 桌面端通知入口不要挤压搜索框、语言按钮、发起活动按钮和头像菜单。
...
19. feature/chat-notification-polish
```

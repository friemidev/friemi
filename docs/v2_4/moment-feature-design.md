...
- 用户发布 Moment：文字、图片，图片可 1-6 张。
- Moment 信息流：优先展示好友动态，自己的动态也展示。
- 点赞 / 取消点赞。
- 评论 / 删除自己的评论。
- 转发：支持站内转发公开足迹；外部分享保留为分享链接 / OG 卡片。
- 举报 Moment 或评论。
- 点赞、评论、站内转发产生站内通知。
...
| `/{locale}/footprints/{momentId}` | Moment 详情页    | 已新增，用于通知精准跳转、分享和举报目标预览 |
...
Moment 不建议复用 `Activity` 或 `Comment` 表。活动评论绑定活动，Moment 评论应独立建模。当前实现还包含 `repostCount` 与 `resharedMomentId`，站内转发会创建一条新的 Moment，并引用原始 Moment。
...
同时保留 `公开` 作为并排按钮，用户可以主动选择。信息流会展示自己、好友动态和公开动态。
...
| A 转发 B 的公开 Moment | B          | `MOMENT_REPOSTED`      |
...
文件仍存入 Supabase public bucket，但路径使用：

```text
moments/{userId}/{uuid}.{ext}
```

上传限制建议：

- 代码当前支持最多 6 张；产品后续确认后可扩到 9 张。
- 单张最大 4MB。
- 格式：JPG、PNG、WebP。
- 服务端统一校验 MIME 和大小。
...
- [x] 新增 `MomentVisibility` enum。
- [x] 新增 `Moment` model。
- [x] 新增 `MomentImage` model。
- [x] 新增 `MomentLike` model。
- [x] 新增 `MomentComment` model。
- [x] `NotificationType` 增加 `MOMENT_LIKED`、`MOMENT_COMMENTED`、`MOMENT_COMMENT_REPLY`。
- [x] `ReportTargetType` 增加 `MOMENT`、`MOMENT_COMMENT`。
- [x] `Notification` 增加 `momentId`、`momentCommentId` 可空字段和索引。
- [x] 生成 Prisma migration。

### 11.2 查询与权限

- [x] `getMomentFeed`：返回自己、好友和公开的未删除 Moment。
- [x] `getMomentDetail`：校验可见性。
- [ ] `momentVisibility` 服务：集中判断是否可看、可互动、可举报。
- [ ] 分页使用 cursor，不一次性拉全量。
- [x] 查询返回 likedByMe、likeCount、commentCount。

### 11.3 发布

- [x] `createMoment` server action。
- [x] 文本最大长度建议 500 字。
- [x] 图片最多 6 张。
- [x] 没文字且没图片时禁止发布。
- [x] 图片上传失败时不创建空 Moment。
- [x] 发布成功后 revalidate `footprints`。

### 11.4 点赞

- [x] `toggleMomentLike` server action。
- [x] 唯一约束防重复点赞。
- [x] 给 Moment 作者发 `MOMENT_LIKED` 通知。
- [x] 自己点赞自己不发通知。
- [x] UI 乐观更新。

### 11.5 评论

- [x] `createMomentComment` server action。
- [x] 评论最大长度建议 500 字。
- [x] 支持一级评论；回复评论保留数据库和 action 能力。
- [x] 支持删除自己的评论；Moment 作者也可隐藏评论。
- [x] 给 Moment 作者发 `MOMENT_COMMENTED` 通知。
- [x] 回复别人评论时发 `MOMENT_COMMENT_REPLY`。
- [x] 自己评论自己不发通知。
- [x] 卡片评论入口打开评论 sheet。
- [x] Feed 内显示最近评论，评论过多时跳详情页加载更多。

### 11.6 转发

- [x] 分享按钮调用 Web Share API。
- [x] 不支持时复制链接。
- [x] 分享入口指向 `/{locale}/footprints/{momentId}`。
- [x] 分享卡片文案 / OG metadata 精修。
- [x] 公开足迹支持站内转发动态。
- [x] 转发数乐观更新。
- [x] 转发通知精准跳转到原足迹详情页。
...
- [x] ReportDialog 支持 `MOMENT`。
- [x] ReportDialog 支持 `MOMENT_COMMENT`。
- [x] Moment 卡片更多菜单接入举报。
- [x] 评论更多菜单接入举报。
- [x] 管理后台 Report 列表能显示 Moment 目标摘要。

### 11.8 通知

- [x] `getNotificationCopy` 增加 Moment 文案。
- [x] `getNotificationPath` 增加 Moment 详情跳转。
- [x] 通知中心展示 Moment 互动。
- [x] Android / iOS 推送文案覆盖点赞和评论。
- [x] Android / iOS 推送文案覆盖站内转发。
- [x] 未读角标计数复用现有 `Notification.readAt`。

### 11.9 UI

- [x] 新增 `/{locale}/footprints` 页面。
- [x] 移动端底部导航“消息”替换为“足迹”。
- [x] 足迹顶部 tab：Moment / Message / Profile。
...
- [x] 发布、点赞、评论、举报全部服务端鉴权。
- [x] 删除 Moment 使用软删除 `deletedAt`。
- [x] 被删除 Moment 不再出现在 feed。
- [x] 举报后不自动删除内容，由管理后台处理。
- [x] 图片上传限制格式和大小。
- [x] 防止用户查看非好友 Moment。
...
- [ ] B 点赞 A，A 收到站内通知。
- [ ] B 评论 A，A 收到站内通知。
- [ ] B 转发 A 的公开足迹，A 收到站内通知。
- [ ] A 自己点赞 / 评论自己不发通知。
- [x] 举报 Moment 后管理后台可见。
- [ ] 移动端底部导航进入足迹。
- [ ] Message tab 可以进入消息。
- [ ] Profile tab 可以进入或展示个人主页摘要。
- [x] Android App 内点击通知能回到对应 Moment。
...
v0.6：站内转发 + 评论 sheet + 足迹专用上传路径 + OG 分享卡片
```

...
- [x] 所有用户可见文案只面向用户，不写给管理者或开发人员
- [x] 用户文案保持简洁、可靠、自然，避免解释功能实现或露出 AI 味
...
- [x] 顶部显示已解锁数量
- [x] 最近解锁放第一屏
- [x] 成就墙按分组展示：开始、参与、组织、身份、特殊
- [x] 未解锁成就低调展示
- [x] 每个成就包含图标、名称、短说明、解锁时间或进度
- [x] 读取 `UserAchievement`
- [x] 读取活动参与数、举办数、信用值、身份字段计算进度
- [x] 他人公开 Profile 只读取已解锁成就，不读取未解锁进度
...
- [x] 展示礼物目录
- [x] 礼物卡片显示 emoji、名称、魅力值、价格占位
- [x] 节日礼物显示暂未开放
- [x] 负分礼物不展示
- [x] 点击礼物可查看详情
- [x] 暂不购买
- [x] 可以引导到送礼入口，但最终送礼仍使用现有 `CharmGiftDialog`
- [x] 页面不出现真实支付入口

### 8.4 本 PR 验收标准

- [x] 新用户能看到成就初始状态
- [x] 已参加过活动的用户能看到对应成就或进度
- [x] 共创主理人能看到特殊身份成就
- [x] 新用户能看到欢迎支票
- [x] 没有道具时展示简洁空状态
- [x] 已使用或过期的支票不会显示为可用
- [x] 碎片不足 10 个时不能兑换盲盒支票
- [x] 三语言礼物名称正常
- [x] 未开放礼物不能送出
- [x] 商城没有真实支付入口
- [x] `npm run typecheck --workspace=apps/web` 通过
...
- [x] 新增 canonical 路由 `app/[locale]/lobby/[activityId]/room/page.tsx`
- [x] 活动详情页 CTA 区或参与者区增加“群聊”按钮
- [x] 移动端优先打开独立路由，便于返回和加载
- [x] 桌面端可在详情页新增紧凑入口，空间不足时进入同一路由
- [x] 顶部显示活动标题和参与状态
- [x] 中间展示最近 50 条消息
- [x] 底部固定输入栏
- [x] 键盘弹起时输入栏不被遮挡
...
- [x] 未登录：显示登录按钮，并保留 redirect
- [x] 非参与者：显示加入活动提示，不显示消息
- [x] 纯公共采集活动：显示创建/加入用户组局提示，不显示公共活动群聊
- [x] 待审核：显示等待审核提示
- [x] 已取消/已拒绝：显示不可访问提示
- [x] 活动取消：历史消息只读
- [x] 活动结束：历史消息只读
...
- [x] Organizer 可看可发
- [x] Co-manager 可看可发
- [x] `JOINED / APPROVED` 参与者可看可发
- [x] `PENDING / REJECTED / CANCELLED` 不可看消息内容
- [x] 游客不可看消息内容
- [x] 纯公共采集活动没有群聊
- [x] 活动结束后不能继续发送
- [x] 删除权限只能由本人、Organizer、Co-manager 使用
- [x] 移动端输入栏不被 bottom nav 或键盘遮挡
- [x] `npm run typecheck --workspace=apps/web` 通过
- [x] `npm test --workspace=apps/web` 通过
- [x] `git diff --check` 通过
...
- [x] 他人 Profile Header 增加消息按钮
- [x] Profile 访客列表增加消息入口
- [x] 活动主理人区复用消息入口
- [x] 活动参与者列表复用消息入口
- [x] Moment 作者头像或更多操作可进入消息
- [x] 所有入口调用同一套服务层策略
- [x] 新增入口都带 `redirectPath`，未登录后能回到原页面
- [x] Server action 不吞掉登录跳转，游客登录后仍回到原入口
- [x] 非好友发满 2 条后仍可从入口打开已有会话

### 10.2 会话页提示

- [x] 非好友且对方未回复：顶部显示剩余次数
- [x] 剩余 2 条：提示语轻
- [x] 剩余 1 条：提示更明确
- [x] 剩余 0 条：禁用输入，显示等待对方回复
- [x] 对方回复后隐藏限制提示
- [x] 好友会话不展示限制提示
- [x] 图片消息发送后同样扣减本地剩余次数
- [x] 发送失败后重试也同步本地剩余次数
...
- [x] 未登录：跳登录并保留 redirect
- [x] 低信用：解释当前不能主动私聊陌生人
- [x] 好友快捷发送失败时：引导先加好友
- [x] 两条已满：提示等待对方回复
- [x] 服务端错误不只显示通用失败
- [x] 用户可见文案保持短句，不使用开发/管理口吻

### 10.4 本 PR 验收标准

- [x] 非好友第 1 条消息可发送
- [x] 非好友第 2 条消息可发送
- [x] 非好友第 3 条消息在对方未回复前被拦截
- [x] 对方回复后双方可继续聊天
- [x] 好友不受两条限制
- [x] 低信用用户不能主动给非好友发消息
- [x] 图片消息不能绕过两条限制
...
- [x] 用户签到确认后触发“真实参加”活动成就
- [x] 如果活动没有启用签到，使用活动已结束且参与状态为 `JOINED / APPROVED` 作为兜底
- [x] 活动创建后触发“第一次组织活动”
- [x] 活动状态变更为完成/结束后同步参与/举办次数成就
- [x] 用户身份更新后同步共创主理人和 Trusted 成就
- [x] Profile 成就页打开时可做轻量兜底同步
- [x] 兜底同步失败不阻塞页面展示

### 13.2 邀请触发

- [x] 首次访问含 `ref` 的页面时记录 referral cookie
- [x] 注册/首次创建 Profile 时绑定邀请关系
- [x] 好友请求接受后补 `friendshipAcceptedAt`
- [x] 被邀请者第一次真实参加活动后写 `firstParticipationAt`
- [x] 好友邀请成功继续触发信用值 `INVITE_FRIEND`
- [x] 不因单纯打开邀请链接发奖励
- [x] 不因同一 invitee 重复建立/解除好友关系重复奖励

### 13.3 背包触发

- [x] 新注册用户继续发放欢迎 Friemi 支票
- [x] 成功组局后发放盲盒碎片
- [x] 碎片满 10 时允许兑换盲盒支票
- [x] 活动不是 `PUBLIC_EVENT`
- [x] 活动未取消
- [x] 活动结束时间已过
- [x] 有至少 2 个有效真人参与，包括 organizer
- [x] 有效真人包括 organizer、`JOINED / APPROVED` 注册参与者、已 linked 的游客报名
- [x] 不计入 `PENDING / REJECTED / CANCELLED` 参与者
- [x] 同一组织者同一活动只发一次碎片
- [x] 同一活动对应的碎片事件必须有稳定 `sourceKey`

...
| 首页轻量 select | 待提交 `feature/playwright-performance-up-v1.1` | 继续压缩 `/home` 的 `home.activities` | Vercel Logs：`activityCount=7 previewLimit=8`，但 `home.activities` 仍出现 3063ms、4281ms、9275ms、9676ms、10217ms、10320ms，说明不是展示数量问题，而是首页查询 select/count 偏重 | 首页预览查询改用轻量 select；不再查询参与者头像、收藏数、公开活动 team count；activity 表只取 legacy public activity info，避免普通组局参与者/count 进入首页；返回纯公开活动卡片数据 | 本地验证通过：typecheck、单元测试、home Playwright；local dev desktop home 4.6s。Preview 待复测 | `npm run typecheck --workspace=apps/web`、`npm run test --workspace=apps/web`、`PLAYWRIGHT_MONITOR_WORKERS=1 npm run monitor:site --workspace=apps/web -- --grep home` | 本地完成，待 Preview 复测 |
...
| `/zh-CN/home` desktop | Preview 约 2.6s 到 2.7s；用户反馈 `/fr/home` 桌面体感仍慢且只展示 4 个 | 第一版 Preview：desktop home 4.5s，`home.activities` 最高约 10.32s；二次轻量 select 本地 home Playwright 4.6s，Preview 待测 | 首页数量/未开始展示已完成；性能瓶颈仍在 `home.activities`，已继续减轻 select | 部署后看 `/fr/home` Playwright 和 Vercel `[perf] route=/home` 的 `home.activities` 是否降到稳定 2-3s |
...
| 待提交 | 首页预览改用轻量 select，去掉参与者头像、收藏数、公开活动 team count 等首屏不展示的数据 | `getActivities.ts` | Local home monitor passed，desktop home 4.6s；Preview 待测 `home.activities` |

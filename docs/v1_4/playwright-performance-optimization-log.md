...
| 登录态组队大厅首屏按需加载 | 待提交 `feature/playwright-performance-up-v1.1` | 解决登录后首次进入 `/lobby` 偶发 10s+ loading | 用户反馈：登录后切到组队大厅加载超过 10 秒，随后切活动页和主页瞬间完成；判断首个登录态大厅请求承担了过多非当前首屏数据 | `/lobby` 登录态 SSR 从 `getActivityLobby()` 改为 `getActivityLobbyInitial()`；首屏只取默认大厅、开放局、我发起、我参加；我收藏、好友发起、好友参加通过 `deferredFilters` 和 `/api/lobby/section` 在用户点击对应筛选时加载；日志标签从 `lobby.fullData` 改为 `lobby.initialData` | 本地验证通过：public desktop lobby 5.5s，mobile lobby 2.4s；Preview 需重点看登录态 `[perf] route=/lobby` 是否不再出现 10s+，并确认日志字段为 `lobby.initialData` | `npm run typecheck --workspace=apps/web`、`npm run test --workspace=apps/web`、`PLAYWRIGHT_MONITOR_WORKERS=1 npm run monitor:site --workspace=apps/web -- --grep lobby` | 本地完成，待 Preview 复测 |
...
| 登录态 `/zh-CN/lobby` | 用户反馈登录后首次进入大厅偶发 10s+，随后其他页面瞬间完成 | 本地已改为首屏 `lobby.initialData`，非首屏分区 deferred loading；公开 lobby monitor 通过 | 慢点不是页面资源，而是登录态大厅 SSR 等了非首屏分区 | 部署后在 Vercel Logs 搜 `[perf] route=/lobby`，确认 `lobby.initialData` 耗时和是否仍有 10s+ |
...
| 待提交 | 登录态组队大厅首屏只加载 initial 分区，收藏和好友相关分区延迟到点击后请求 | `lobby/page.tsx` | 本地 lobby monitor 通过：desktop public lobby 5.5s、mobile lobby 2.4s；Preview 需观察 `lobby.initialData` |

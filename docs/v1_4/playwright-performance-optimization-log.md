...
| 登录态 `/zh-CN/lobby` | 用户反馈登录后首次进入大厅偶发 10s+，随后其他页面瞬间完成；后续 Preview logs 实测 `/fr/lobby total=18338ms` | 本地已改为首屏 `lobby.initialData`，并关闭登录态常驻导航的重路由预取 | 慢点不只是页面资源或大厅数据，还叠加了导航层并发预取 | 部署后在 Vercel Logs 搜 `[perf] route=/lobby`，确认 `lobby.initialData` 和同秒并发路由数量是否下降 |
...
| 待提交 | 登录态组队大厅首屏只加载 initial 分区，收藏和好友相关分区延迟到点击后请求；同时关闭常驻导航对 `/lobby`、`/activities`、`/messages`、`/profile`、`/activities/new`、`/notifications` 的自动预取 | `lobby/page.tsx`、`DesktopNav.tsx`、`MobileNav.tsx`、`NotificationHeaderLink.tsx`、`AccountMenu.tsx`、`AppHeader.tsx` | 下一轮 Preview 应不再出现 `/fr/lobby total=18338ms` 且同秒并发拉起多条重量路由 |

...
- 不接入 Expo / React Native / Capacitor，也不在规划分支直接引入 Kotlin 工程依赖
- 不改现有后端 API 结构
- 不新增数据库表
...
### 路线更新：Android 长期使用 Kotlin

随着 Android APP 目标从临时 WebView 壳升级为产品化 APP，后续会做原生扫码签到、原生通知中心、原生聊天、图片 / 表情包能力和桌游工具原生化。因此 Android 后续开发路线调整为：

```text
现有 Java WebView 壳继续维护
  -> 新增原生能力统一使用 Kotlin
  -> 高频场景逐步沉淀为 Kotlin 原生模块
  -> 必要时再评估是否把壳工程整体迁移为 Kotlin
```

执行约定：

- 当前 Java 壳不为了语言纯度立刻重写，避免破坏已经跑通的登录、Deep Link 和 WebView 基础体验。
- 新增 FCM、扫码签到、通知中心、聊天、桌游工具和复杂原生页面时，不再继续扩展 Java。
- Kotlin 模块稳定后，再单独拆迁移分支处理 `MainActivity.java` 等壳代码。
- Android 和 iOS 可以保持各自原生路线；Android 侧以 Kotlin 作为长期主语言。
...
后续如果发现 WebView / TWA 在推送、扫码、相册、桌游工具或性能上无法满足体验，优先把局部页面迁移到 Kotlin 原生模块；React Native / Expo 暂不作为当前主路线。
...
- 不作为第一步重写全站，但作为 Android 新增原生能力的默认语言
- 先保留 WebView 壳，逐步用 Kotlin 承接扫码、推送、通知、聊天和桌游工具

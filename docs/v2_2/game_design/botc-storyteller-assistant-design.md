...
当前分支先把说书人工具接入 Friemi 桌游工具平台底座，而不是直接做完整魔典。

已落地：

- [x] `GameToolKind` 预留 `STORYTELLER`
- [x] `/game-tools` 统一桌游工具大厅展示说书人工具入口
- [x] `/game-tools/storyteller` 说书人工具预览页说明共用房间、私密魔典和公共屏规划
- [x] 短房号、私密 token、默认座位名和房间 revalidate 已抽成通用 `gameToolRooms.ts`
- [x] 游戏工具展示信息已抽成 `gameToolCatalog.ts`，说书人工具与 Avalon 共用入口配置

暂未落地：

- [ ] 说书人正式创建房间
- [ ] 魔典座位盘、身份标签、提醒物和说书人备注
- [ ] 夜晚流程清单
- [ ] 提名、投票、死亡和鬼票记录
- [ ] 公共屏正式页面
...
说书人工具的隐藏状态复杂度远高于 Avalon，所以 v1.0 只先共用房间底座。后续正式做魔典时，必须继续保持“隐藏信息只在说书人端，不进入公共屏或玩家端”的边界。

...
## 0.6.2 v1.0-alpha 落地状态：同步、队长提名与开局前座位管理

当前分支已开始落实 v1.0-alpha，目标是让真实线下局更顺：状态同步更可感知，当前队长可以自己在手机上提名队伍，房主可以在开局前处理错座和链接问题。

v1.0-alpha 已覆盖：

- [x] 自动刷新从隐藏脉冲升级为可见同步状态：房间页、私密玩家页和公共屏显示“同步中 / 已同步 / 离线”
- [x] 页面聚焦、恢复前台、重新联网时会主动刷新，后台时降低刷新频率，减少移动端无意义消耗
- [x] 当前队长的私密页在提队阶段展示队伍选择面板，玩家可直接勾选本轮队伍并提交
- [x] 队长提名服务端校验当前私密 token 对应座位是否等于 `currentLeaderSeatNumber`，不能只靠前端显示控制
- [x] 房主保留房间页代提名入口，用于线下主持代操作或玩家手机不可用时兜底
- [x] 房主候场阶段新增座位管理：重命名座位、释放误认领座位、刷新私密身份链接
- [x] 座位管理只允许房主在 `LOBBY` 状态执行，操作写入 `GameToolEvent`

v1.0-alpha 仍不包含：

- [ ] WebSocket / Supabase Realtime 级别的真正推送同步；当前是更稳定、更可见的短轮询方案
- [ ] 开局后强制换座或完整座位交换；当前只处理开局前释放、改名和刷新链接
- [ ] 防重复提交后的“等 X 人 / 可修改”完整状态面板
- [ ] 重开本局 / 重新发身份
- [ ] 断线重连专用入口

结论：v1.0-alpha 已经解决三个最影响真实桌面局节奏的问题：大家知道页面是否在同步，队长不用把手机交给房主代操作，开局前错座可以快速修正。beta 应继续补重开 / 重新发牌、防重复提交反馈和断线重连。

## 0.6.3 v1.0-beta 落地状态：重开、重新发牌与重连反馈

v1.0-beta 继续解决真实桌面局里的事故恢复问题：身份泄露、误发身份、玩家重复点击、手机切后台或网络短断。目标不是增加复杂主持后台，而是让一局线下游戏在出错后能快速回到可玩状态。

v1.0-beta 已覆盖：

- [x] 房主房间页新增“重开”操作区，和普通轮次修正分开，避免把危险操作混在同一组按钮里
- [x] “回候场”保留座位和玩家，清空身份、提交、进度和胜负状态，房主可重新开局
- [x] “重新发牌”保留座位和私密链接，重新洗身份并从第 1 轮重新开始，玩家刷新原链接即可看到新身份
- [x] 两个危险操作都需要二次确认，并写入 `GameToolEvent`，事件轨道能看到 `room_reopened` / `room_redealt`
- [x] 投票、任务牌和刺杀目标重复提交时返回“已记录”提示，不再显示成系统错误
- [x] 服务端对 `GameToolSubmission` 唯一约束冲突做兜底处理，避免快速连点造成异常报错
- [x] 同步状态支持手动点击刷新，离线时移动端直接显示离线状态；页面聚焦、恢复前台、重新联网时主动刷新

v1.0-beta 仍不包含：

- [ ] 真正实时推送通道；当前仍是增强短轮询 + 手动同步
- [ ] 完整暂停 / 恢复游戏状态；暂停会影响所有玩家遮罩和权限，后续应作为房主控制能力单独设计
- [ ] 提交后修改自己的选择；当前只防重复，不允许回改

结论：v1.0-beta 已经覆盖身份类游戏最常见的事故恢复链路。下一步 rc 可以继续补任务洗牌揭晓、基础复盘页和公共屏细节。

...
import { z } from "zod";
import {
  bindPlatformCouponTemplate,
  createAdminCouponTemplate,
} from "@/features/coupons/adminCoupons";
...
const couponSchema = z.union([
  z.object({
    platformTemplateKey: z.string().trim().min(1).max(80),
  }),
  z.object({
...
export async function POST(
  ...
  try {
    const coupon =
      "platformTemplateKey" in parsed.data
        ? await bindPlatformCouponTemplate(
            merchantId,
            parsed.data.platformTemplateKey,
          )
        : await createAdminCouponTemplate(merchantId, parsed.data);

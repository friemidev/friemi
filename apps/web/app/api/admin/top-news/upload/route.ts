...
import {
  createSignedTopNewsImageUpload,
  finalizeSignedTopNewsImageUpload,
  getActivityCoverStorageConfig,
  uploadTopNewsImageBuffer,
...
export async function POST(request: Request) {
  ...
  if (request.headers.get("content-type")?.includes("application/json")) {
    ...
    const result =
      body.action === "create"
        ? await createSignedTopNewsImageUpload(userId, {
            name: body.fileName,
            size: body.fileSize,
            type: body.fileType,
          })
        : await finalizeSignedTopNewsImageUpload(userId, body.path);
  ...
  const uploaded = await uploadTopNewsImageBuffer(
    userId,

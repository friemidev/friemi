...
import {
  createSignedProfileAvatarUpload,
  finalizeSignedProfileAvatarUpload,
...
export async function POST(request: Request) {
  ...
  if (request.headers.get("content-type")?.includes("application/json")) {
    ...
    const result =
      body.action === "create"
        ? await createSignedProfileAvatarUpload(userId, {
            name: body.fileName,
            size: body.fileSize,
            type: body.fileType,
          })
        : await finalizeSignedProfileAvatarUpload(userId, body.path);

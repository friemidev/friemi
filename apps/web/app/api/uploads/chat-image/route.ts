...
export async function POST(request: Request) {
  ...
  const uploaded = await uploadChatImageBuffer(
    userId,
    validated.fileBuffer,
    validated.detectedMimeType,
  );
  if ("error" in uploaded) return uploadError(uploaded.error, 500);

  return NextResponse.json({ path: uploaded.path, url: uploaded.url });
}

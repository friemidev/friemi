...
import {
  createSignedDirectMessageImageUpload,
  finalizeSignedDirectMessageImageUpload,
...
export async function POST(request: Request) {
  ...
  if (request.headers.get("content-type")?.includes("application/json")) {
    ...
    const result =
      body.action === "create"
        ? await createSignedDirectMessageImageUpload(userId, {
            name: body.fileName,
            size: body.fileSize,
            type: body.fileType,
          })
        : await finalizeSignedDirectMessageImageUpload(userId, body.path);

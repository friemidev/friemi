  ...
  return {
    ...
    summary: payload.summary
      ? {
          ...
          balanced_out_candidates: payload.summary.balancedOutCandidates,
          candidate_fetched: payload.summary.candidateFetched,
          created: payload.summary.created,
          duplicate_candidates: payload.summary.duplicateCandidates,
          duration_mix: payload.summary.durationMix.map((bucket) => ({
          ...
          pools: payload.summary.pools.map((pool) => ({
            balanced_out: pool.balancedOut,
            fetched: pool.fetched,
            key: pool.key,
            label: pool.label,
            limit: pool.limit,
            selected: pool.selected,
            unique_candidates: pool.uniqueCandidates,
          })),
          ...
          time_windows: payload.summary.timeWindows.map((window) => ({
            balanced_out: window.balancedOut,
            duplicate_candidates: window.duplicateCandidates,
            end_at: window.endAt,
            duration_buckets: window.durationBuckets.map((bucket) => ({
              fetched: bucket.fetched,
              key: bucket.key,
              label: bucket.label,
              selected: bucket.selected,
            ...
            unique_candidates: window.uniqueCandidates,
          })),

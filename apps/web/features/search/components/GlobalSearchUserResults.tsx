    ...
    <div className="space-y-3">
      ...
      {canExpand ? (
        <div className="flex flex-wrap items-center gap-2">
          <button
            type="button"
            className="inline-flex h-9 items-center justify-center rounded-full bg-white/85 px-3.5 text-sm font-semibold text-[#315b48] ring-1 ring-[#cfe2c6] transition hover:bg-white"
            onClick={() =>
              setExpanded((current) => {
    ...
    <article className="flex min-w-0 flex-col gap-4 rounded-xl border border-sand bg-white/80 p-4 shadow-sm sm:flex-row sm:items-center">
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-1 inline-flex max-w-full items-center rounded-full bg-team-bg px-2.5 py-1 text-xs font-medium text-[#315b48] ring-1 ring-[#cfe2c6]">
...
function AddFriendSubmitButton({ locale }: { locale: string }) {
  ...
  return (
    <button
      type="submit"
      className={cn(
        "inline-flex h-10 w-full items-center justify-center gap-2 rounded-full bg-[#049d73] px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-[#006e4d] disabled:pointer-events-none disabled:opacity-60",

...
export function ActivityWeatherWidget({
  ...
  if (state.status === "loading") {
    return (
      <section
        className={cn(
          "rounded-[1.1rem] border border-[#cfe2c6] bg-white/72 p-3 shadow-sm sm:p-4",
          className,
        )}
      >
        <div className="flex items-center gap-2 text-sm font-semibold text-ink">
          <LoaderCircle className="h-4 w-4 animate-spin text-[#049d73]" />
          {t.loading}
        </div>
        <div className="mt-3 h-12 rounded-xl bg-gradient-to-r from-[#f1faeb] via-white to-[#e8f3e2]" />
      </section>
    );
  }
  ...
  if (state.status === "unavailable") {
    return (
      <section
        className={cn(
          "rounded-[1.1rem] border border-[#cfe2c6] bg-white/58 p-3 text-sm text-zinc-500 sm:p-4",
          className,
        )}
      >
        <div className="flex items-center gap-2">
          <CloudSun className="h-4 w-4 text-[#049d73]" />
  ...
  return (
    <section
      className={cn(
        "rounded-[1.1rem] border border-[#cfe2c6] bg-[#f7fff3] p-3 text-sm text-zinc-700 shadow-sm sm:p-4",
        className,
      )}
      aria-label={t.title}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 font-semibold text-ink">
            <Icon className="h-4 w-4 text-[#049d73]" />
            {t.title}
          </div>
        ...
        <span className="shrink-0 rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-[#315b48] ring-1 ring-[#bfd6b7]">
          {date.slice(5)}
        </span>
      </div>

      <div className="mt-3 flex items-center justify-between gap-3 rounded-xl bg-white/76 p-3 ring-1 ring-[#cfe2c6]">
        <div>
        ...
        <ThermometerSun className="h-8 w-8 shrink-0 text-[#049d73]" />
      </div>

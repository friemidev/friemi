...
export function WechatWebViewGuide({ locale }: WechatWebViewGuideProps) {
  ...
  return (
    <section className="fixed inset-0 z-[100] overflow-y-auto bg-[#FEFFF9] px-6 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4">
      <BrandBackdrop
      ...
      <div className="mx-auto flex min-h-[calc(100svh-2rem)] w-full max-w-xs flex-col py-8">
        <div className="space-y-6 pt-[7svh]">
          ...
          <div className="relative z-10 space-y-2">
            {copy.steps.map((step, index) => (
              <div
                key={step}
                className="grid grid-cols-[1.5rem_minmax(0,1fr)] gap-2 rounded-lg border border-[#D6D5B2] bg-white/86 p-2.5"
              >
        ...
        <div className="relative z-10 mt-auto space-y-3 pt-8">
          <p className="rounded-lg bg-[#F1F2E3] px-3 py-2 text-xs leading-5 text-moss">
            {copy.fallback}
          </p>
          <Button
            type="button"

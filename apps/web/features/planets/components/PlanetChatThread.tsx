...
export function PlanetChatThread({
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      ...
      const isComposing = Boolean(document.activeElement?.closest("[data-planet-chat-composer]"));
      const hasDraft = input instanceof HTMLInputElement && Boolean(input.value.trim());
      ...
      {messages.map((message, index) => {
        ...
        return (
          <Fragment key={message.id}>
            ...
            <div className={`flex items-start gap-2 ${isViewer ? "flex-row-reverse" : ""}`}>

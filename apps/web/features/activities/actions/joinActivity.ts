...
const joinableActivityVisibility: ActivityVisibility[] = ["PUBLIC", "PRIVATE"];
...
function getJoinFailure(error: string, reasonCode: JoinFailureReasonCode) {
  return {
    ok: false as const,
    error,
    reasonCode,
    ...
    const joinResult = await perf.measure("transaction", () =>
      prisma.$transaction(
        async (tx) => {
          ...
          const profileNormalizedWechatId = normalizeGuestWechatId(
            profile.normalizedWechatId,
          );
          const profileNormalizedPhone = normalizeGuestPhone(
            profile.normalizedPhone,
          );
          const profileNormalizedEmails = new Set(
            [
              normalizeGuestEmail(profile.normalizedContactEmail),
          ...
          for (const profileNormalizedEmail of profileNormalizedEmails) {
            guestDuplicateConditions.push({
          ...
          if (profileNormalizedPhone) {
            guestDuplicateConditions.push({
              normalizedPhone: profileNormalizedPhone,
            });
          }

          if (guestDuplicateConditions.length > 0) {
            const existingGuestRecord =
            ...
            if (existingGuestRecord) {
              return getJoinFailure(
                "你已以游客身份报名过，请在个人页面绑定邮箱、电话或微信号来关联记录。",
                "already_joined",
              );
            }
          }

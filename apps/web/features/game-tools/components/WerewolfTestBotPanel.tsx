...
function getCopy(locale: string) {
  if (locale === "fr") {
    return {
      ...
      finishGood: "Village",
      finishGoodConfirm:
        "Terminer cette partie de test avec le village gagnant ?",
...
export function WerewolfTestBotPanel({
  ...
  return (
    <form
      action={action}
      className="overflow-hidden rounded-[1.4rem] border border-[#F1F2E3] bg-[#19181A] p-4 text-white shadow-[0_16px_44px_rgba(30,23,24,0.16)]"
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full bg-[#F1F2E3] px-2.5 py-1 text-[11px] font-bold text-[#1E1718]">
            <img
              alt=""
              aria-hidden="true"
              className="h-4 w-4"
      ...
      {room.status === "LOBBY" ? (
        <div className="mt-4 grid gap-2">
          <TestBotButton
            className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-[#F1F2E3] px-4 text-sm font-bold text-[#1E1718] transition hover:bg-[#F1F2E3] disabled:cursor-not-allowed disabled:opacity-55"
